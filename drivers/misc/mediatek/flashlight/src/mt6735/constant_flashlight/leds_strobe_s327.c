...
#define GPIO_CAMERA_FLASH_MODE GPIO42
#define GPIO_CAMERA_FLASH_EN   GPIO80 //GPIO43
...
int leds_strobe_set_pwm( void)
{
	struct pwm_spec_config pwm_setting;
	memset(&pwm_setting, 0, sizeof(struct pwm_spec_config));
	if(g_duty){
				pwm_setting.pwm_no = 4;//PWM4
				pwm_setting.mode = PWM_MODE_OLD; 
				pwm_setting.pmic_pad = false;
				pwm_setting.clk_div = CLK_DIV1;//CLK_DIV128
				pwm_setting.clk_src = PWM_CLK_OLD_MODE_BLOCK;//PWM_CLK_OLD_MODE_32K;
				pwm_setting.PWM_MODE_OLD_REGS.IDLE_VALUE = 0;
				pwm_setting.PWM_MODE_OLD_REGS.GUARD_VALUE = 0;
				pwm_setting.PWM_MODE_OLD_REGS.GDURATION = 0;
				pwm_setting.PWM_MODE_OLD_REGS.WAVE_NUM = 0;
				pwm_setting.PWM_MODE_OLD_REGS.DATA_WIDTH = 8000;//25
				pwm_setting.PWM_MODE_OLD_REGS.THRESH = 8000;//10 5 //8
		}else{
				...
				pwm_setting.PWM_MODE_OLD_REGS.GDURATION = 0;
				pwm_setting.PWM_MODE_OLD_REGS.WAVE_NUM = 0;
				pwm_setting.PWM_MODE_OLD_REGS.DATA_WIDTH = 1000;//25
				pwm_setting.PWM_MODE_OLD_REGS.THRESH = 250;//10 5 //8
	}
...
/* char bufR[2]; */
	...
	readReg(0xb);
#endif
	if(mt_set_gpio_mode(GPIO_CAMERA_FLASH_EN,GPIO_MODE_05)){PK_DBG("[constant_flashlight] set gpio mode failed!!\n");}
	leds_strobe_set_pwm();
...
int FL_Init(void)
{
	...
	if(mt_set_gpio_mode(GPIO_CAMERA_FLASH_MODE,GPIO_MODE_00)){PK_DBG("[constant_flashlight] set gpio mode failed!!\n");}
...
*/
	...
	case FLASH_IOC_SET_ONOFF:
		...
		break;
//ken add flashlight ATA test
	case FLASH_IOC_TORCH_ONOFF:
		PK_DBG("FLASHLIGHT_TORCH_ONOFF: %d\n", (int)arg);
		if (arg == 1) {
			FL_Enable();
		} else {
			FL_Disable();

...
static ssize_t test_get(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	struct cust_mt65xx_led *cust_led_list = mt_get_cust_led_list();
		...
		red = pmic_get_register_value(PMIC_ISINK_CH0_EN);
		green = pmic_get_register_value(PMIC_ISINK_CH1_EN);
		blue = mt_mt65xx_led_get_cust(&cust_led_list[MT65XX_LED_TYPE_BLUE]);
  ...
  flag |= blue \
	...
	/*	 
	flag |= pmic_get_register_value(PMIC_ISINK_CH2_EN) \
		 |  pmic_get_register_value(PMIC_ISINK_CH1_EN)<<1 \
		 |  pmic_get_register_value(PMIC_ISINK_CH0_EN)<<2;
	*/

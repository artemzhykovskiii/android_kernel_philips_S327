...
int mt_mt65xx_led_get_cust(struct cust_mt65xx_led *cust)
{
	 int led_status= 0;
	if (cust->mode == MT65XX_LED_MODE_GPIO){
		 led_status=gpio_get_value(cust->data);
		 //LEDS_DEBUG("mt_mt65xx_led_get_cust MT65XX_LED_MODE_GPIO \n");
	}
...
int mt_mt65xx_led_set_cust(struct cust_mt65xx_led *cust, int level)
{
	...
	case MT65XX_LED_MODE_GPIO:
		...
		//printk( "brightness_set_cust:go GPIO mode %x !!!!!\n",level);

		gpio_request(cust->data,"uard_urxd2");
		gpio_direction_output(cust->data,0);
		gpio_set_value(cust->data,level); 
		gpio_free(cust->data);

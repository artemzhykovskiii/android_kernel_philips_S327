...
int8_t ekg_status[1024];
int8_t ppg1_status[1024];
int8_t ppg2_status[1024];
...
int ekg_data_report(struct bio_data *data)
{
	...
	cxt->bio_data.get_data_ekg(ekg_data, ekg_status, &length);
	/* BIO_ERR("%s ====> length:%d\n", __func__, length); */
...
int ppg1_data_report(struct bio_data *data)
{
	...
	cxt->bio_data.get_data_ppg1(ppg1_data, ppg1_amb_data, ppg1_agc_data, ppg1_status, &length);
...
int ppg2_data_report(struct bio_data *data)
{
	...
	cxt->bio_data.get_data_ppg2(ppg2_data, ppg2_amb_data, ppg2_agc_data, ppg2_status, &length);
	/* BIO_ERR("%s ====> length:%d\n", __func__, length); */

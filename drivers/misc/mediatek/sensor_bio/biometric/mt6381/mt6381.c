...
int mt6381_i2c_write(u8 addr, u8 *buf, u16 length)
{
	...
	mt6381_i2c_client->timing = 300;
	mt6381_i2c_client->ext_flag = 0;
	...
	case VSM_SRAM_EKG:
		{
			...
			if (err == VSM_STATUS_OK) {
				...
				err = vsm_driver_read_register(&data);
#if 0
				pr_warn("ekg_sample_data1 0x%x, ekg_sample_data2 0x%x",
		...
		if (sram_type == VSM_SRAM_PPG2 && amb_buf != NULL) {
			...
			for (i = 0; i < sram_len; i++) {
				...
				if (amb_read_counter % 64 == 0) {
					...
					/* BIO_ERR("AMB: read_counter = %d, amb_data[%d] = %d\n", read_counter,
						i, amb_buf[i] >= 0x400000 ? amb_buf[i] - 0x800000 : amb_buf[i]); */
				} else
...
static enum vsm_status_t vsm_driver_read_offline_mode_data(enum vsm_signal_t type, uint32_t *data_buf, u32 *len)
{
	...
	*len = 0;
	...
	if (index < 0) {
		BIO_ERR("Wrong type %d\n", type);
		return VSM_STATUS_INVALID_PARAMETER;
	}

	if (olm_info_p == NULL) {
		BIO_ERR("olm_info_p == NULL\n");
		return VSM_STATUS_ERROR;
	}

	if (olm_info_p->sensor[index].filp == NULL) {
		BIO_ERR("olm_info_p->sensor[index].filp == NULL\n");
		return VSM_STATUS_ERROR;
	}
...
int32_t vsm_driver_chip_version_get(void)
{
	...
	if (vsm_chip_version == -1) {
		...
		err = vsm_driver_read_register(&data);
		if (err == VSM_STATUS_OK) {
			pr_warn("read back chip version:0x%x", read_data);
...
static int MT6381_CaliReadSram(enum vsm_sram_type_t sram_type, int32_t *data_buf, uint32_t len)
{
	...
	if (sram_len > 0) {
		...
		err = vsm_driver_read_register_batch(sram_type, (u8 *) data_buf, sram_len);
		if (err < 0)
			BIO_ERR("vsm_driver_read_register_batch fail, %d\n", err);
	}
	...
	while ((cali_sram1_buf != NULL && sram1_len != 0) || (cali_sram2_buf != NULL && sram2_len != 0)) {
		if (cali_sram1_buf != NULL && sram1_len != 0) {
			...
			if (read_len > sram1_len) {
				BIO_ERR("Wrong len, sram1_len = %d, read_len = %d\n", sram1_len, read_len);
				return -1;
			}
			sram1_len -= read_len;
		}
		if (cali_sram2_buf != NULL && sram2_len != 0) {
			...
			if (read_len > sram2_len) {
				BIO_ERR("Wrong len, sram2_len = %d, read_len = %d\n", sram2_len, read_len);
				return -1;
			}
			sram2_len -= read_len;
		}
...
static int MT6381_DoCalibration(struct biometric_cali *cali)
{
	...
	if (atomic_read(&bio_trace) != 0) {
		BIO_ERR("sumOfambdac = %lld, count = %d\n", sumOfambdac, count);
		BIO_ERR("pga6 = %d, ambdac5_5 = %d\n", cali->pga6, cali->ambdac5_5);
	}
	if (cali->pga6 < 6000 || cali->pga6 > 6060) {
		BIO_ERR("pga fail, %d is not between 6000~6060.\n", cali->pga6);
		return -1;
	}
	if (cali->ambdac5_5 < 20957234 || cali->ambdac5_5 > 22296629) {
		BIO_ERR("ambdac fail, %d is not between 20957234~22296629.\n", cali->ambdac5_5);
		return -1;
	}
	return 0;
}
...
static int MT6381_WriteCalibration(struct biometric_cali *cali)
{
	...
	BIO_ERR("pga6 = %d, ambdac5_5 = %d, dc_offset = %lld\n", pga6, ambdac5_5, dc_offset);
	return 0;
}
...
int mt6381_get_data_ekg(int *raw_data, int8_t *status, u32 *length)
{
	int i;
...
static long mt6381_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (err) {
		BIO_ERR("access error: %08X, (%2d, %2d)\n", cmd, _IOC_DIR(cmd), _IOC_SIZE(cmd));
		return -EFAULT;
	}
...
static int mt6381_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (err) {
		BIO_ERR("mt6381_device misc register failed!\n");

...
//static struct gyro_hw *get_cust_gyro(void)
//{
//	return &gyro_cust;
//}
...
int mpu_i2c_read_block(struct i2c_client *client, u8 addr, u8 *data, u8 len)
{
	int err;
	u8 beg = addr;
...
int mpu_i2c_write_block(struct i2c_client *client, u8 addr, u8 *data, u8 len)
...
int MPU6050_hwmsen_read_block(u8 addr, u8 *buf, u8 len)
{
	if (NULL == mpu6050_i2c_client) {
		GYRO_ERROR("MPU6050_hwmsen_read_block null ptr!!\n");
...
{
	if (NULL == mpu6050_i2c_client) {
		GYRO_ERROR("MPU6050_hwmsen_write_block null ptr!!\n");
		return MPU6050_ERR_I2C;
	}
...
#ifdef MPU6050_ACCESS_BY_GSE_I2C
	if (MPU6050_hwmsen_read_block(MPU6050_REG_DEVID, databuf, 0x01)) {
#else
	if (hwmsen_read_byte(client, MPU6050_REG_DEVID, databuf)) {
#endif
		GYRO_ERROR("read power ctl register err!\n");
		return MPU6050_ERR_I2C;
	}

	sprintf(buf, "MPU6050 Chip ID: 0X%X", databuf[0]);
	return 0;
}
...
static int mpu6050_get_data(int *x , int *y, int *z, int *status)
{
	...
	*status = SENSOR_STATUS_ACCURACY_MEDIUM;
	GYRO_DBG("get gyro data lpp:%x %x %x\n", *x, *y, *z);
	return 0;
}

...
#endif
/*
Match the tp FW according to LCM name.
it's only support for msg5846 TP driver.
*/
int tpv_lcm_name_to_detect_tp(void)
{
	LCM_DRIVER *lcm_drv = NULL;
	char lcd_name[3][35]={{"hx8394d_hd720_dsi_vdo_tpv_new"},{"ili9881c_hd720_dsi_vdo_ski"},{"ili9881c_hd720_dsi_vdo_akj"}};
	
	lcm_drv = lcm_driver_list[index];
	if (!strcmp(lcd_name[0], lcm_drv->name)){
		return HX8394D_VDO_TPV_NEW;
	}else if (!strcmp(lcd_name[1],  lcm_drv->name)){
		return ILI9881C_VDO_SKI;
	}else if (!strcmp(lcd_name[2],  lcm_drv->name)){
		return ILI9881C_VDO_AKI;
	}

	return 0;
}
EXPORT_SYMBOL(tpv_lcm_name_to_detect_tp);

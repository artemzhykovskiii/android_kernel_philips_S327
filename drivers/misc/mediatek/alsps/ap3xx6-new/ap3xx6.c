...
int ap3xx6_setup_eint(struct i2c_client *client)
{
	...
	if (ap3xx6_obj->irq_node) {
		...
		err = request_irq(ap3xx6_obj->irq, ap3xx6_eint_handler, IRQF_TRIGGER_FALLING, "ALS-eint", NULL);
...
static ssize_t ap3xx6_em_read(struct device_driver *ddri, char *buf)
{
	...
	int reg_value[1];
		...
		u8 ap3xx6_NUM_CACHABLE_REGS = sizeof(ap3xx6_reg)/sizeof(ap3xx6_reg[0]);
#else		
	...
	count += sprintf(buf+count, "[ps raw data]=%d\n", idx);
	return count;
}
...
#if defined(CONFIG_OF)	
	obj->irq_node = of_find_compatible_node(NULL, NULL, "mediatek, als-eint");//als_ps
#endif
...
APS_LOG("als_register_data_path OK.%s:, %d\n", __func__, __LINE__);

...
static int stk3x1x_enable_als(struct i2c_client *client, int enable)
{
	...
	APS_LOG("%s: enable=%d, obj->state_val=%d\n", __func__, enable, old);
...
//	obj->hw = get_cust_alsps_hw();
	...
	atomic_set(&obj->state_val, 0x0);//obj->hw->state_val
	atomic_set(&obj->psctrl_val, 0x71);//obj->hw->psctrl_val
	atomic_set(&obj->alsctrl_val, 0x38);//obj->hw->alsctrl_val
	obj->ledctrl_val = 0xFF;	//obj->hw->ledctrl_val
	obj->wait_val = 0x7;			//obj->hw->wait_val
	obj->int_val = 0;
	obj->first_boot = true;			 
	obj->als_correct_factor = 1000;

...
static imgsensor_info_struct imgsensor_info = { 
  ...
  .custom5 = {
	...
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_Gr,//sensor output first pixel color SENSOR_OUTPUT_FORMAT_RAW_Gb
...
////////////////////////////////////////////////
	...
	write_cmos_sensor(0x0000, 0x0200); //image orient GR
...
static int check_is_2nd_source(struct hi553_otp_struct otp_info)
{
	if((otp_info.module_integrator_id==0x4)&&(otp_info.lens_id==0x4)&&(otp_info.sensor_id==0x53))
		return 1;
	else return 0;
}
...
static int hi553_otp_read(void)
{
	...
	checksum = (hi553_otp.module_integrator_id + hi553_otp.AF_Flag + hi553_otp.prodyction_year + hi553_otp.production_month + hi553_otp.production_day + hi553_otp.sensor_id + hi553_otp.lens_id 
		 +hi553_otp.vcm_id + hi553_otp.Driver_ic_id +hi553_otp.F_num_id) % 0xFF + 1;
    ...
    if(checksum == hi553_otp.infocheck)
	{
		...
		LOG_INF("wangk_HI553_Sensor: Module information checksum PASS\n ");
	}else{
		LOG_INF("HI553_Sensor: Module information checksum Fail\n ");
		LOG_INF("wangk_HI553_Sensor: Module information checksum Fail\n ");
	}
	LOG_INF("HI553 checksum = 0x%x\n", checksum);
	...
	if(wb_start_addr != 0){
		...
		for(i = 0; i < 30; i++){
			...
			LOG_INF("HI553 wb_data[%d] = 0x%x\n", i, hi553_otp.wb_data[i]);
		}
	}
...
static int hi553_otp_apply(void)
{
	...
	if((hi553_otp.infocheck ==0)&&(hi553_otp.sensor_id == 0)&&(hi553_otp.lens_id == 0)&&(hi553_otp.Driver_ic_id == 0))
		return 0;
	...
	if(check_is_2nd_source(hi553_otp)){
		RG_ratio_golden = 0x158;//(hi553_otp.wb_data[6] << 8) | (hi553_otp.wb_data[7] & 0x03FF);
		BG_ratio_golden = 0x123;//(hi553_otp.wb_data[8] << 8) | (hi553_otp.wb_data[9] & 0x03FF);
	}
	else{
		RG_ratio_golden = 0x152;//(hi553_otp.wb_data[6] << 8) | (hi553_otp.wb_data[7] & 0x03FF);
		BG_ratio_golden = 0x135;//(hi553_otp.wb_data[8] << 8) | (hi553_otp.wb_data[9] & 0x03FF);
	}
	wbcheck = hi553_otp.wb_data[29];
	checksum = ((hi553_otp.wb_data[0] + hi553_otp.wb_data[1] + hi553_otp.wb_data[2] + hi553_otp.wb_data[3] ) & 0xFF) + 1;

    if(checksum == wbcheck)
	{
		LOG_INF("HI553_Sensor: WB checksum PASS\n ");
		LOG_INF("wangk_HI545_Sensor: WB checksum PASS\n ");
	}
    else{
		LOG_INF("HI553_Sensor: WB checksum Fail\n ");
		LOG_INF("wangk_HI553_Sensor: WB checksum Fail\n ");
	}
	LOG_INF("HI553 checksum = 0x%x\n", checksum);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    kal_uint8 retry = 2;
    int  addr = 0;
    struct hi553_otp_struct hi553_otp_info;
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            if (return_sensor_id() == imgsensor_info.sensor_id) {
    ...
    LOG_INF("zxczxczxc sensor start init\n");
    sensor_init();
    LOG_INF("zxczxczxc sensor end init\n");
    {
		otp_init_setting();

		hi553_otp_info.Base_Info_Flag = OTP_read_cmos_sensor(0x501);
		if(hi553_otp_info.Base_Info_Flag == 0x01)	//Base Info Group1 valid
			addr = 0x502;
		else if(hi553_otp_info.Base_Info_Flag == 0x13)	//Base Info Group2 valid
			addr = 0x513;
		else if(hi553_otp_info.Base_Info_Flag == 0x37)	//Base Info Group3 valid
			addr = 0x524;
		else
			addr = 0;
		LOG_INF("zxczxczxc 1111111111111\n");
		// if(addr==0)
		// 	return ERROR_NONE;
		LOG_INF("zxczxczxc 222222222222\n");
		hi553_otp_info.module_integrator_id = OTP_read_cmos_sensor(addr);
		hi553_otp_info.AF_Flag = OTP_read_cmos_sensor(addr + 1);
		hi553_otp_info.prodyction_year = OTP_read_cmos_sensor(addr + 2);
		hi553_otp_info.production_month = OTP_read_cmos_sensor(addr + 3);
		hi553_otp_info.production_day = OTP_read_cmos_sensor(addr + 4);
		hi553_otp_info.sensor_id = OTP_read_cmos_sensor(addr + 5);
		hi553_otp_info.lens_id = OTP_read_cmos_sensor(addr + 6);
		hi553_otp_info.vcm_id = OTP_read_cmos_sensor(addr + 7);
		hi553_otp_info.Driver_ic_id = OTP_read_cmos_sensor(addr + 8);
		hi553_otp_info.F_num_id = OTP_read_cmos_sensor(addr + 9);
		hi553_otp_info.infocheck = OTP_read_cmos_sensor(addr + 16);

		write_cmos_sensor_byte(0x0a00, 0x00); //sleep On
		mdelay(10);
		write_cmos_sensor_byte(0x003f, 0x00); //OTP mode off
		write_cmos_sensor_byte(0x0a00, 0x01); //sleep Off
  LOG_INF("zxczxczxc HI553 module_integrator_id = 0x%x, AF_Flag = 0x%x, prodyction_year = 0x%x, production_month = 0x%x, production_day = 0x%x, sensor_id = 0x%x, lens_id= 0x%x, infocheck = 0x%x\n",\
		hi553_otp_info.module_integrator_id,hi553_otp_info.AF_Flag, hi553_otp_info.prodyction_year, hi553_otp_info.production_month, hi553_otp_info.production_day, hi553_otp_info.sensor_id, hi553_otp_info.lens_id, hi553_otp_info.infocheck);



		if(check_is_2nd_source(hi553_otp_info))
			{
				sensorid=0x553a;
				*sensor_id=0x553a;
				//imgsensor_info.sensor_id=0x553a;
				LOG_INF("zxczxczxc 3333333333333\n");
			}
			else
				*sensor_id=0x553;
	}
...
static kal_uint32 open(void)
{
    ...
    LOG_1;
    //sensor have two i2c address 0x6c 0x6d & 0x21 0x20, we should detect the module used i2c address
    LOG_INF("V377_hi553_open_161122 \n");
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        spin_lock(&imgsensor_drv_lock);
        imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
        spin_unlock(&imgsensor_drv_lock);

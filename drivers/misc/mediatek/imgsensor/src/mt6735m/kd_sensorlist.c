...
MUINT32 g_invokeSensorId[KDIMGSENSOR_MAX_INVOKE_DRIVERS]={0};
...
int kdSetDriver(unsigned int *pDrvIndex)
{
	...
	for (i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS; i++) {
		...
		if (MAX_NUM_OF_SUPPORT_SENSOR > drvIdx[i]) {
			...
			/* return sensor ID */
			g_invokeSensorId[i]=pSensorList[drvIdx[i]].SensorId;
		...
		for (i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS; i++) {
			if (DUAL_CAMERA_NONE_SENSOR != g_invokeSocketIdx[i]) {
				...
				if (sensorID == 0) {	/* not implement this feature ID */
					...
					if(sensorID!=g_invokeSensorId[i]){
						err=ERROR_SENSOR_CONNECT_FAIL;
						goto SENSOR_ID_NOT_MATCH;
					}

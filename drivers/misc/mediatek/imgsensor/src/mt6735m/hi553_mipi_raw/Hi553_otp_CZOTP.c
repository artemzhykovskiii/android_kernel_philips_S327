...
signed char check_MID_CZOTP_hi553(bool bOnlyCheck);
...
bool otp_update_wb_CZOTP_hi553(unsigned short golden_rg, unsigned short golden_bg) 
{
	...
	if(DEBUG_PRINT)
	printk("[Hi553 ]enter otp_update_wb_CZOTP_hi553 r_gain=0x%x b_gain = 0x%x  \n",r_gain,b_gain);
...
signed char check_MID_CZOTP_hi553(bool bOnlyCheck)
{
		...
		if(DEBUG_PRINT)
		printk("[Hi553 ]enter check_MID_CZOTP_hi553g =0x%x \n",bOnlyCheck);

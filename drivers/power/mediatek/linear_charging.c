...
void select_charging_curret(void)
{
	if (g_ftm_battery_flag) {
		...
		if (BMT_status.charger_type == STANDARD_HOST) {
			...
			g_temp_CC_value = batt_cust_data.ac_charger_current;
			/*sf jinfeng changed, debpend on hardware id pin to changed charing current 20170728*/
			if (strncmp(CONFIG_ARCH_MTK_PROJECT, "tpv6737m_35g_n_s327", 19) == 0)
			{
				if (mt_get_gpio_in_base(GPIO76))
				{
					if (BMT_status.bat_vol < 3500)
						g_temp_CC_value = CHARGE_CURRENT_700_00_MA;
					else if (BMT_status.bat_vol < 3700)
						g_temp_CC_value = CHARGE_CURRENT_800_00_MA;
					else
						g_temp_CC_value = CHARGE_CURRENT_1000_00_MA;
				}

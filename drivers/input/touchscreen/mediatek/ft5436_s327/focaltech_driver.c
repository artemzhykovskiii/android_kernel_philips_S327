...
int ft6x36_detected = 0;//lihonghua add
unsigned char firmware_version=0;

/*sf jinfeng add for glove mode start 20170425*/
int glove_mode_flag =0;

#ifdef FTS_TP_NAME //linhui
u8 ft6x36_sensor_id = 0x07;
...
unsigned int touch_irq = 0;
 //touch type
unsigned short detect_touch_type=0;
...
static irqreturn_t tpd_eint_interrupt_handler(unsigned irq, struct irq_desc *desc);//change by zby
...
//	mutex_lock(&i2c_access);
	...
	memcpy(g_dma_buff_va, writebuf, writelen);
	printk("fts_i2c_Write client->addr =%x\n",client->addr);
	client->addr = (client->addr & I2C_MASK_FLAG) | (I2C_DMA_FLAG);
	if((ret=i2c_master_send(client, (unsigned char *)g_dma_buff_pa, writelen))!=writelen)
...
void focaltech_get_upgrade_array(void)
{
	...
	{
		memcpy(&fts_updateinfo_curr, &fts_updateinfo[10], sizeof(struct Upgrade_Info));//lihonghua modify 20160926
	}
}
/*
*  tpd_need_rotate(int *x, int *y)
*  This function can  correct the angle when the TP mirroring 
*  @ x : Vertical active vaule according to SPEC
*  @ y : Horizontal active vaule according to SPEC
*/ 
static void tpd_need_rotate(int *x, int *y)
{
 	*x = (TPD_RES_X + 1)  - *x;
	*y = (TPD_RES_Y + 1)  - *y;	
}
...
static  void tpd_down(int x, int y, int p) {
	...
	if(detect_touch_type == TG_TP_I2C_ADDDRESS){
		tpd_need_rotate(&x,&y);
	}	
	if(x > TPD_RES_X)
	{
...
//	u8 report_rate =0;
	...
	mutex_lock(&i2c_access);
       ...
       reg[0] = 0x00;
	fts_i2c_Read(i2c_client, reg, 1, data, 32);//64);
...
//		 mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM); 
 ...
 static irqreturn_t tpd_eint_interrupt_handler(unsigned irq, struct irq_desc *desc)
 {
	 ...
	 TPD_DEBUG_PRINT_INT;
	 tpd_flag = 1;
	 disable_irq_nosync(touch_irq);//zby add change
	 wake_up_interruptible(&waiter);
   return IRQ_HANDLED;
 }
...
static ssize_t tp_name_proc_read_proc(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
	...
	switch(ft6x36_sensor_id)
	{
		case 0x15:
			ptr += sprintf(ptr, "TP:FT6X36_CW_0x%2x\n",firmware_version);
			break;
		case 0x69:
			ptr += sprintf(ptr, "TP:FT6X36_HXD_0x%2x\n",firmware_version);
			break;
		case 0x67:
			ptr += sprintf(ptr, "TP:FT6X36_DJ_0x%2x\n",firmware_version);
			break;
		case 0xDA:
			ptr += sprintf(ptr, "TP:FT6X36_DJ_0x%2x\n",firmware_version);
			break;
		default:
			ptr += sprintf(ptr, "TP:FT6X36_0x%2x\n",firmware_version);
			break;
	}
...
//detect the touch type 
int touch_i2c_detect(struct i2c_client *client)
{
	unsigned char buf[2];
	int retval=0;
	int i =0;
	int lengh = sizeof(buf);

	buf[0] = 0xff;
	buf[1] = ((1>>8)&0xff);
	for(i=0;i<3;i++){
		retval = i2c_master_send(client, buf, lengh);
		if(retval==lengh){		
			detect_touch_type=HF_TP_I2C_ADDDRESS; 
			printk("%s the client->addr=0x%x detect_touch_type=%d\n",__FUNCTION__,client->addr,detect_touch_type);
			return detect_touch_type;
		}
		msleep(10);
	}
	client->addr = TG_TP_I2C_ADDDRESS;
	for(i=0;i<3;i++){
		retval = i2c_master_send(client, buf, lengh);
		if(retval==lengh){		
			detect_touch_type=TG_TP_I2C_ADDDRESS;
			printk("%s the client->addr=0x%x detect_touch_type=%d\n",__FUNCTION__,client->addr,detect_touch_type);
			return detect_touch_type;
		}
		msleep(10);
...
//EXPORT_SYMBOL_GPL(touch_i2c_detect)

/*read the boot partion of tp to detect the vendor ID*/
static int  fts_ft5x46_get_vendor_id_flash(struct i2c_client *client,u8 *pVendorID)
{
	...
	if(i_ret == 0)
	{
		TPD_DMESG("[Eric_FTS	FTS 8 ] hid change to i2c fail ! \n");
	}
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		if (i_ret < 0)
		{
			TPD_DMESG("%s line =%d [Eric_FTS FTS 10] Upgrade Step 1 reset i2c transfer error\n",__FUNCTION__,__LINE__);
		}
		...
		if(i_ret == 0)
		{
			TPD_DMESG("[%s line =%d Eric_FTS	FTS 12 ] hid change to i2c fail ! \n",__FUNCTION__,__LINE__);
			//continue;
		}
		msleep(10);
		...
		if(i_ret < 0)
		{
			TPD_DMESG("[%s line =%d [Eric_FTS FTS 13] Upgrade Step 1 reset i2c transfer error\n",__FUNCTION__,__LINE__);
			//continue;
		}
		...
		if (reg_val[0] == fts_updateinfo_curr.upgrade_id_1
			...
			TPD_DMESG("%s line =%d [Eric_FTS FTS 15] Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",
				  __FUNCTION__,__LINE__, reg_val[0], reg_val[1]);
			break;
		} else {
			TPD_DMESG("%s line =%d [Eric_FTS FTS 16] Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n", __FUNCTION__,__LINE__,reg_val[0], reg_val[1]);
	...
	packet_buf[0] = 0x03;
	packet_buf[1] = 0x00;
	packet_buf[2] = (u8)(CONFIG_VENDOR_ID_ADDR>>8);
	packet_buf[3] = (u8)(CONFIG_VENDOR_ID_ADDR);

	i_ret=fts_i2c_Read(client, packet_buf, sizeof(packet_buf), pVendorID, 1);
	TPD_DMESG("%s line =%d vendor_id =%x\n", __FUNCTION__,__LINE__,*pVendorID);
	if((i_ret < 0) ||(0 == *pVendorID)){
		return -EIO;
	}
...
//	client->addr=0x38
	...
	printk("[fts6x36] tpd_probe i2c_client \n");
	...
	msleep(200);
	retval=touch_i2c_detect(i2c_client);
	if(retval<0)
	{
		dev_err(&client->dev,"%s: Failed to test i2c\n",__func__);
		return retval;
	}
#if 0  //lanjing mark
...
//    	fts_init_gpio_hw();
	...
	fts_i2c_Write(i2c_client, uc_reg_addr, 1);
	fts_i2c_Read(i2c_client, uc_reg_addr, 0, uc_reg_value, 1);
	firmware_version=uc_reg_value[0];
	...
	TPD_DEBUG("mtk_tpd[FTS] chip id is %d.\n",uc_reg_value[0] );
	//lihonghua add for Compatible begin
	printk("mtk_tpd[FTS] chip id is %d.\n",uc_reg_value[0]);
	if(uc_reg_value[0]!=0x54){
		printk("mtk_tpd [FTS] read chip id failed.\n");
		tpd_load_status = 0;
		return retval;
	}
	else{
		ft6x36_detected=1;
		printk("mtk_tpd [FTS] read chip id successful.\n");
	}
	//lihonghua add for Compatible end
...
//    mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
	...
	//enable_irq(touch_irq);
	disable_irq(touch_irq);//change by zby
    ...
    hall_state_menu_proc = proc_create(HALL_STATE_PROC_FILE, 0666, NULL, &hall_state_menu_proc_fops);
	...
	thread = kthread_run(touch_event_handler, 0, TPD_DEVICE);
	 if (IS_ERR(thread))
    	...
    	printk("focaltech_get_upgrade_array\n");
	focaltech_get_upgrade_array();
	//read the boot partion of tp vaule to detect vender ID
	for(i=0;i<5;i++){
		if(0==fts_ft5x46_get_vendor_id_flash(i2c_client,&vendor_id)){
			 ft6x36_sensor_id=vendor_id;
			break;
		}
		msleep(5);	
    	}
	TPD_DMESG("mtk_tpd[FTS]  vendor_id from boot =0x%x\n",vendor_id);
#ifdef SYSFS_NODE
...
#endif
//lihonghua add for FW upgrade begin 20160926
#ifdef TPD_AUTO_UPGRADE

		printk("********************Enter CTP Auto Upgrade********************\n");
		DBG("[Eric_FTS FTS 1 ] upgrade 0  \n");
		if((ft6x36_sensor_id==TPV_TP_SENSOR_ID) ||(ft6x36_sensor_id==HF_TP_SENSOR_ID) ||(detect_touch_type==TG_TP_I2C_ADDDRESS)){
			fts_ctpm_auto_upgrade(i2c_client);
		}
		DBG("[Eric_FTS	FTS 2 ] upgrade 1  \n");
#endif
//lihonghua add for FW upgrade end 20160926
	...
	fts_write_reg(i2c_client, 0xc0, 0x0);// jinfeng add for close glove mode 20170505
...
//  	TPD_DMESG("Focaltech fts I2C Touchscreen Driver (Built %s @ %s)\n", __DATE__, __TIME__);
  if (get_boot_mode() == LOW_POWER_OFF_CHARGING_BOOT
	...
	if (retval != 0) {
		printk("[fts6x36]Failed to set reg-vgp6 voltage: %d\n", retval);
		return -1;
	}
...
//	mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);  
	...
	/*sf jinfeng add for glove mode start 20170425*/
	if (glove_mode_flag == 1)
		fts_write_reg(i2c_client, 0xc0, 0x1);
	else
		fts_write_reg(i2c_client, 0xc0, 0x0);
	/*sf jinfeng add for glove mode end 20170425*/
...
//	msleep(5);
	...
	if (retval != 0)
		printk("[fts6x36]Failed to disable reg-vgp6: %d\n", retval);
#else

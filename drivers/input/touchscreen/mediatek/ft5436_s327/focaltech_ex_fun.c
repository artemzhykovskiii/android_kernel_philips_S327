...
static unsigned char CTPM_FW[]={
	#include "FT5x46_S327_V07_D01_20170427_app.i"
};

static unsigned char CTPM_FW_HXD[] = {
	#include "FT5x46_S327_V07_D01_20170427_app.i"
};
...
static unsigned char CTPM_FW_SKI[]={
	#include "TG1388-0x72_Ref_V07_D01_20170706_app.i"
};

static unsigned char CTPM_FW_HXD_SKI[] = {
	#include "TG1388-0x72_Ref_V07_D01_20170706_app.i"
};

static unsigned char CTPM_FW_DJ_SKI[] = {
	#include "TG1388-0x72_Ref_V07_D01_20170706_app.i"
};

static unsigned char CTPM_FW_HC55297_SKI[]={
	#include "HC55297_SKI550_FT5436_ID0X79_Ver0X04_HD_20171016_app.i"
};
extern u8 ft6x36_sensor_id;
extern unsigned char firmware_version;

extern int glove_mode_flag;
extern int detect_touch_type;
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	*/
	...
	{
		if(ft6x36_sensor_id==0x67){
				fw_len=sizeof(CTPM_FW_DJ);
		}else if(ft6x36_sensor_id==HF_TP_SENSOR_ID){
			if(detect_touch_type == HF_TP_I2C_ADDDRESS)
				fw_len=sizeof(CTPM_FW_HC55297_SKI);
			else if(detect_touch_type == TG_TP_I2C_ADDDRESS)
				fw_len=sizeof(CTPM_FW_HXD_SKI);	
		}else if(ft6x36_sensor_id==TPV_TP_SENSOR_ID){
			if(detect_touch_type == TPV_TP_I2C_ADDDRESS)
				fw_len=sizeof(CTPM_FW_HXD);
			else if(detect_touch_type == TG_TP_I2C_ADDDRESS)
				fw_len=sizeof(CTPM_FW_HXD_SKI);	
		}
	...
	if(detect_touch_type == HF_TP_I2C_ADDDRESS){
        	if(ft6x36_sensor_id==0x67){
			pbt_buf = CTPM_FW_DJ;
			i_ret = fts_5x46_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_DJ));
		}else if(ft6x36_sensor_id==HF_TP_SENSOR_ID){
			pbt_buf = CTPM_FW_HC55297_SKI;
			i_ret = fts_5x46_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_HC55297_SKI));
		}else if(ft6x36_sensor_id==TPV_TP_SENSOR_ID){
			pbt_buf = CTPM_FW_HXD;
		...
		if (i_ret != 0)
		 ...
		 if(ft6x36_sensor_id==0x67){
			pbt_buf = CTPM_FW_DJ_SKI;
			i_ret = fts_5x46_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_DJ_SKI));
        	}else if(ft6x36_sensor_id==TG_TP_SENSOR_ID){
			pbt_buf = CTPM_FW_HXD_SKI;
			i_ret = fts_5x46_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_HXD_SKI));
		}
		if (i_ret != 0)
			dev_err(&client->dev, "%s:upgrade failed. err.\n",
					__func__);
...
u8 fts_ctpm_get_i_file_ver(void)
{
  ...
  if(detect_touch_type == HF_TP_I2C_ADDDRESS){
	if(ft6x36_sensor_id==0x67)
		ui_sz = sizeof(CTPM_FW_DJ);
	else if(ft6x36_sensor_id==HF_TP_SENSOR_ID)
		ui_sz = sizeof(CTPM_FW_HC55297_SKI);
	else if(ft6x36_sensor_id==TPV_TP_SENSOR_ID)
	...
	if (ui_sz > 2)
	{
	    if(fts_updateinfo_curr.CHIP_ID==0x36)
			...
			if(ft6x36_sensor_id==0x67)
				return CTPM_FW_DJ[ui_sz - 2];
			else if(ft6x36_sensor_id==HF_TP_SENSOR_ID)
				return CTPM_FW_HC55297_SKI[ui_sz - 2];
			else if(ft6x36_sensor_id==TPV_TP_SENSOR_ID)
	...
	if(ft6x36_sensor_id==0x67)
		ui_sz = sizeof(CTPM_FW_DJ_SKI);
	else if(ft6x36_sensor_id==TG_TP_SENSOR_ID)
		ui_sz = sizeof(CTPM_FW_HXD_SKI);
	if (ui_sz > 2)
	{
	    if(fts_updateinfo_curr.CHIP_ID==0x36)
                return CTPM_FW_SKI[0x10a];
	    else{
			if(ft6x36_sensor_id==0x67)
				return CTPM_FW_DJ_SKI[ui_sz - 2];
			else if(ft6x36_sensor_id==TG_TP_SENSOR_ID)
				return CTPM_FW_HXD_SKI[ui_sz - 2];
	    }
	}
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	if (/*the firmware in touch panel maybe corrupted */
		...
		if (i_ret == 0)	{
			msleep(300);
			uc_host_fm_ver = fts_ctpm_get_i_file_ver();
			firmware_version = uc_host_fm_ver;
...
ft_fw_reflash_4:
	...
	for (j = 0; j < packet_number; j++) {
		...
		for(i = 0;i < 30;i++)
		{
			...
			// jinfeng remove fix auto update phone reboot. 20170411
...
static ssize_t fts_ftsglovemode_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	ssize_t num_read_chars = 0;
	...
	if (fts_read_reg(client, 0XC0, &fwver) < 0)
		num_read_chars = snprintf(buf, FT_PAGE_SIZE,
					"get tp glove mode fail!\n");
	else
		num_read_chars = snprintf(buf, FT_PAGE_SIZE, "%d\n", fwver);
...
static ssize_t fts_ftsglovemode_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	struct i2c_client *client = container_of(dev, struct i2c_client, dev);
	...
	retval = kstrtoul(buf, 0, &valbuf);

	mutex_lock(&g_device_mutex);
	if (valbuf == 1)
	{
		fts_write_reg(client, 0xC0, 0x1);// open glove mode
		glove_mode_flag = 1;
	}
	else
	{
		fts_write_reg(client, 0xC0, 0x0);
		glove_mode_flag = 0;
	}
	mutex_unlock(&g_device_mutex);
...
static ssize_t fts_ftsreset_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	/*place holder for future use*/
...
static ssize_t fts_ftsreset_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	//struct i2c_client *client_dev = container_of(dev, struct i2c_client, dev);
	if(!strncmp(buf,"1",1)){
...
/*read the  fts sensor id node */
static ssize_t fts_ftssensorid_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	static u8 sensor_id =0;
	if(detect_touch_type==HF_TP_I2C_ADDDRESS){
		if(ft6x36_sensor_id==TPV_TP_SENSOR_ID) 
			sensor_id=1;//TPV_TP
		else if (ft6x36_sensor_id==HF_TP_SENSOR_ID) 
			sensor_id=2;//HF_SKI_TP
	}else if(detect_touch_type==TG_TP_I2C_ADDDRESS){
		if(ft6x36_sensor_id==TG_TP_SENSOR_ID) 
			sensor_id=3;//TG_SKI_TP
	}
	return sprintf(buf,"%d\n",sensor_id);
}

static ssize_t fts_ftssensorid_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	/*place holder for future use*/
...
/*sf jinfeng add for glove mode start 20170425*/
static DEVICE_ATTR(ftsglovemode, S_IRUGO | S_IWUSR, fts_ftsglovemode_show,
			fts_ftsglovemode_store);
/*sf jinfeng add for glove mode end 20170425*/

/*set fts reset node
*read vuale: cat ftsreset 
*write vuale : echo 1 > ftsreset, it  need to reset the tp
*/
static DEVICE_ATTR(ftsreset, S_IRUGO | S_IWUSR, fts_ftsreset_show,
			fts_ftsreset_store);

/*read/write fts sensor id node
*read vuale: cat ftssensorid 
*/
static DEVICE_ATTR(ftssensorid, S_IRUGO | S_IWUSR, fts_ftssensorid_show,
			fts_ftssensorid_store);
...
static struct attribute *fts_attributes[] = {
	...
	&dev_attr_ftsreset.attr, 
	&dev_attr_ftssensorid.attr, 
	NULL
};

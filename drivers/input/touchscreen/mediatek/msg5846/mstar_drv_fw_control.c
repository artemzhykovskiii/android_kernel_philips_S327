...
#include "msg28xx_xxxx_update_bin.h" // for MSG28xx
 #include "msg28yy_yyyy_update_bin.h"
...
extern int tpv_lcm_name_to_detect_tp(void);
/*=============================================================*/
    ...
    if (pPacket[0] == 0x5A)
    {
...
#endif //CONFIG_ENABLE_HOTKNOT  
        {
            for (i = 0; i < MAX_TOUCH_NUM; i ++)
            {
                ...
                //pInfo->tPoint[pInfo->nCount].nY = 1280-nY * TOUCH_SCREEN_Y_MAX / 1900;
...
void _DrvFwCtrlMsg28xxCheckFirmwareUpdateBySwId(void) 
{
    ...
    if (nCrcMainA == nCrcMainB) 
    {
        ...
        {
       	if(1 == tpv_lcm_name_to_detect_tp()){//HX8394D_VDO_TPV_NEW
#ifdef CONFIG_UPDATE_FIRMWARE_BY_TWO_DIMENSIONAL_ARRAY // By two dimensional array
	            nUpdateBinMajor = msg28yy_yyyy_update_bin[127][1013]<<8 | msg28yy_yyyy_update_bin[127][1012]; 
	            nUpdateBinMinor = msg28yy_yyyy_update_bin[127][1015]<<8 | msg28yy_yyyy_update_bin[127][1014];
#else // By one dimensional array
	            nUpdateBinMajor = msg28yy_yyyy_update_bin[0x1FFF5]<<8 | msg28yy_yyyy_update_bin[0x1FFF4];
	            nUpdateBinMinor = msg28yy_yyyy_update_bin[0x1FFF7]<<8 | msg28yy_yyyy_update_bin[0x1FFF6];
#endif
		 }else{
#ifdef CONFIG_UPDATE_FIRMWARE_BY_TWO_DIMENSIONAL_ARRAY // By two dimensional array
        ...
        if (nUpdateBinMinor != nMinor)
        {
            if (eSwId == MSG28XX_SW_ID_XXXX)
            {
                for (i = 0; i < MSG28XX_FIRMWARE_WHOLE_SIZE; i ++)
                {
	                if(1 == tpv_lcm_name_to_detect_tp()){//HX8394D_VDO_TPV_NEW
#ifdef CONFIG_UPDATE_FIRMWARE_BY_TWO_DIMENSIONAL_ARRAY // By two dimensional array
	                    _DrvFwCtrlStoreFirmwareData(msg28yy_yyyy_update_bin[i], 1024);
#else // By one dimensional array
	                    _DrvFwCtrlStoreFirmwareData(&(msg28yy_yyyy_update_bin[i*1024]), 1024);
#endif
	                	}else{
